...
$old_error_handler = set_error_handler("consulta_scriptcase_error_handler");

function consulta_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['consulta']['contr_erro']) && $_SESSION['scriptcase']['consulta']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_consulta->contr_consulta->Apl_com_erro = true;
            if ($inicial_consulta->contr_consulta->NM_ajax_flag)
            {
                $inicial_consulta->contr_consulta->NM_ajax_info['errList']['geral_consulta'][] = $msg;
                return;
            }
            consulta_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function consulta_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['consulta'])  && '' != $_SESSION['scriptcase']['error_icon']['consulta'])  ? $_SESSION['scriptcase']['error_icon']['consulta']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['consulta']) && '' != $_SESSION['scriptcase']['error_close']['consulta']) ? $_SESSION['scriptcase']['error_close']['consulta'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['consulta']) && '' != $_SESSION['scriptcase']['error_span_title']['consulta']) ? $_SESSION['scriptcase']['error_span_title']['consulta'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['consulta']) && '' != $_SESSION['scriptcase']['error_icon_title']['consulta']) ? $_SESSION['scriptcase']['error_icon_title']['consulta'] : '';
?>
