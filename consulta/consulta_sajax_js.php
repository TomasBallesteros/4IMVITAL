...
?>
  ...
  // ---------- Validate codigo
  function do_ajax_consulta_validate_codigo()
  {
    var nomeCampo_codigo = "codigo";
    var var_codigo = scAjaxGetFieldText(nomeCampo_codigo);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_consulta_validate_codigo(var_codigo, var_script_case_init, do_ajax_consulta_validate_codigo_cb);
  } // do_ajax_consulta_validate_codigo

  function do_ajax_consulta_validate_codigo_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_consulta_validate_codigo_cb

  // ---------- Validate imagen
  function do_ajax_consulta_validate_imagen()
  {
    var nomeCampo_imagen = "imagen";
    var var_imagen = scAjaxGetFieldText(nomeCampo_imagen);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_consulta_validate_imagen(var_imagen, var_script_case_init, do_ajax_consulta_validate_imagen_cb);
  } // do_ajax_consulta_validate_imagen

  function do_ajax_consulta_validate_imagen_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "imagen";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_consulta_validate_imagen_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_consulta_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_consulta_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    scAjaxProcOn();
    x_ajax_consulta_submit_form(var_codigo, var_imagen, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_consulta_submit_form_cb);
  } // do_ajax_consulta_submit_form

  function do_ajax_consulta_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_consulta_submit_form_cb_after_alert);
  } // do_ajax_consulta_submit_form_cb
  function do_ajax_consulta_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_consulta_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_consulta_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_consulta_navigate_form, function() {});
    } else {
      perform_ajax_consulta_navigate_form();
    }
  }

  function perform_ajax_consulta_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_consulta_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['consulta']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_consulta_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_consulta_navigate_form_cb_after_alert);
  } // do_ajax_consulta_navigate_form_cb
  function do_ajax_consulta_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_consulta_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_consulta_navigate_form_cb_after_alert

  function sc_hide_consulta_form()
  {
