...
$old_error_handler = set_error_handler("ver_tercero_scriptcase_error_handler");

function ver_tercero_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['ver_tercero']['contr_erro']) && $_SESSION['scriptcase']['ver_tercero']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_ver_tercero->contr_ver_tercero->Apl_com_erro = true;
            if ($inicial_ver_tercero->contr_ver_tercero->NM_ajax_flag)
            {
                $inicial_ver_tercero->contr_ver_tercero->NM_ajax_info['errList']['geral_ver_tercero'][] = $msg;
                return;
            }
            ver_tercero_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function ver_tercero_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['ver_tercero'])  && '' != $_SESSION['scriptcase']['error_icon']['ver_tercero'])  ? $_SESSION['scriptcase']['error_icon']['ver_tercero']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['ver_tercero']) && '' != $_SESSION['scriptcase']['error_close']['ver_tercero']) ? $_SESSION['scriptcase']['error_close']['ver_tercero'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['ver_tercero']) && '' != $_SESSION['scriptcase']['error_span_title']['ver_tercero']) ? $_SESSION['scriptcase']['error_span_title']['ver_tercero'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['ver_tercero']) && '' != $_SESSION['scriptcase']['error_icon_title']['ver_tercero']) ? $_SESSION['scriptcase']['error_icon_title']['ver_tercero'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['ver_tercero']['error_buffer'] .= ob_get_flush();
}
